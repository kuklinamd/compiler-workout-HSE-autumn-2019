...
let rec eval env (cs, ss, (st, i, o)) as config = function
  | [] -> config
  | instr :: instrs ->
    match instr with
    | LABEL _            -> eval env config instrs
    | CALL name          -> eval env (call config instrs) (env#labeled name)
    | JMP label          -> eval env config (env#labeled label)
    | END                -> 
      (match cs with
       | []            -> config
       | (p, st')::cs' -> eval env (cs', ss, (State.drop_scope st st', i, o)) p) 

    | BEGIN (args, lcls) ->
      let st'       = State.push_scope st (args @ lcls) in
      let stack', m = match_args args ss in
      let st'       = List.fold_left (fun st' (a, s) -> State.update a s st') st' m
      in eval env (cs, stack', (st', i, o)) instrs

    | CJMP (cond, label) ->
      let z :: ss' = ss in
      if match_cond cond z
      then eval env config (env#labeled label)
      else eval env config instrs
...
let rec compile_stmt state has_next_op = function
  ...
  | Stmt.Call (name, exprs) ->
    state, List.fold_left (fun acc expr -> compile_expr expr @ acc) [CALL name] exprs
